...
<script>
  ...
  async function abrirWhatsApp() {
    ...
    if (tel1 && tel2) {
      const escolha = await modalEscolherOpcao('Escolher número', 'Selecione qual número deseja usar:', [{ value: '1', label: tel1 }, { value: '2', label: tel2 }]);
  ...
  async function abrirWhatsAppComLink(link) {
    const tel1 = clienteAtual.Telefone1 || '';
    const tel2 = clienteAtual.Telefone2 || '';
  ...
  function preencherProduto(idx, produto) {
    ...
    definirMoedaInput(linha.querySelector('.produtoValorUN'), produto.ValorUnitario || 0);
    definirMoedaInput(linha.querySelector('.produtoValor'), produto.Valor || 0);
  ...
  function atualizarValorProdutoForm(linhaIdx) {
    ...
    const valorUN = valorNumerico(linha.querySelector('.produtoValorUN').value) || 0;
    const valor = un * valorUN;

    definirMoedaInput(linha.querySelector('.produtoValor'), valor);
  ...
  function atualizarValorUnitarioDoTotal(linhaIdx) {
    ...
    const un = parseFloat(linha.querySelector('.produtoUN').value) || 0;
    const valor = valorNumerico(linha.querySelector('.produtoValor').value) || 0;
    
    if (un > 0 && valor > 0) {
      const valorUN = valor / un;
      definirMoedaInput(linha.querySelector('.produtoValorUN'), valorUN);
    }
  ...
  function atualizarValorTotalVenda() {
    ...
    if (inputTotal) {
      definirMoedaInput(inputTotal, total);
    }
  }
  ...
  function gerarParcelasVenda() {
    ...
    const valorTotal = valorNumerico(document.getElementById('vendaValorTotal').value) || 0;
  ...
  async function salvarVenda(vendaIdx, modal, vendaEditando) {
    try {
      ...
      document.querySelectorAll('[id^="produtoLinha"]').forEach(linha => {
        ...
        let valorUN = valorNumerico(linha.querySelector('.produtoValorUN').value) || 0;
        const valor = valorNumerico(linha.querySelector('.produtoValor').value) || 0;
      ...
      for (let i = 0; i < numeroParcelas; i++) {
        ...
        const valor = valorNumerico(document.getElementById(`parcValor${i}`)?.value) || 0;
  ...
  async function deletarVenda(idx) {
    const confirmarExclusao = await modalConfirm('Tem certeza que deseja deletar esta venda?', { title: 'Deletar venda', okText: 'Sim, deletar', cancelText: 'Cancelar' });
    if (!confirmarExclusao) return;

    const senha = await modalPromptSenha('Digite a palavra-chave para confirmar:', '', 'Deletar venda');
    if (senha !== 'Excluir') {
  ...
  async function adicionarObservacao() {
    const texto = await modalPromptObservacao('Digite a observação:', '', 'Nova observação');
    if (!texto || !texto.trim()) {
  ...
  async function removerObservacao(obsId) {
    if (!await modalConfirm('Tem certeza que deseja remover esta observação?', { title: 'Remover observação', okText: 'Remover', cancelText: 'Cancelar' })) return;
  ...
  async function desmarcarClienteFC() {
    const senha = await modalPromptSenha('Digite a palavra-chave para desmarcar FC:\n\n(Mesma palavra usada para deletar)', '', 'Desmarcar FC');
